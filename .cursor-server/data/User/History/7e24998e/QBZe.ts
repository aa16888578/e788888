...
export class CodestralService {
  ...
  constructor() {
    this.apiKey = process.env.CODESTRAL_API_KEY || '';
    this.baseUrl = process.env.CODESTRAL_BASE_URL || 'https://codestral.mistral.ai/v1';
    
    if (!this.apiKey) {
      console.warn('警告: CODESTRAL_API_KEY 環境變數未設置');
    }
  }
