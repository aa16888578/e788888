...
import type { CVVCard } from '@/types/cvv';
...
export default function ApiTestPage() {
  ...
  const testCases = [
    {
      name: '獲取配置選項',
      description: '測試 GET /api/cvv/config',
      test: async () => {
        const config = await cvvService.getConfig();
    ...
    {
      name: '測試 AI 導入 (CSV)',
      description: '測試 POST /api/cvv/import',
      test: async () => {
    ...
    {
      name: '測試 AI 導入 (TXT)',
      description: '測試 POST /api/cvv/import (TXT 格式)',
      test: async () => {
        const result = await cvvService.importCards({
          source: 'api',
          format: 'txt',
          data: '4222222222222222|456|11|2027|CA|18.50',
          options: {
            skipDuplicates: true,
            autoPrice: true,
            tags: ['test', 'txt-format']
          }
        });
        return { success: true, data: result };
      }
    ...
    {
      name: '測試 AI 導入 (JSON)',
      description: '測試 POST /api/cvv/import (JSON 格式)',
      test: async () => {
        const result = await cvvService.importCards({
          source: 'api',
          format: 'json',
