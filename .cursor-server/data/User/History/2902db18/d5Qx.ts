...
export class AgentService {
  ...
  async createAgent(agentData: Partial<Agent>): Promise<Agent> {
    try {
      ...
      if (agent.parentAgentId) {
        await this.addTeamMember(agent.parentAgentId, agent.id, 'direct');
      }

      return agent;
    } catch (error) {
      console.error('創建代理失敗:', error);
      throw new Error('創建代理失敗');
    }
  }
  ...
  async addTeamMember(agentId: string, memberId: string, memberType: 'direct' | 'indirect'): Promise<void> {
    try {
      ...
      await db.collection('agentTeams').add(teamMember);
      
      // 更新代理的團隊人數
      await this.updateAgentStats(agentId);
    } catch (error) {
      console.error('添加團隊成員失敗:', error);
      throw new Error('添加團隊成員失敗');
    }
  }
  ...
  private async calculateTeamSales(agentId: string): Promise<number> {
    try {
      ...
      for (const teamDoc of teamQuery.docs) {
        const teamMember = teamDoc.data() as AgentTeam;
        const memberCommissions = await db.collection('agentCommissions')
          .where('agentId', '==', teamMember.memberId)
          .where('status', 'in', [CommissionStatus.EARNED, CommissionStatus.PAID])
          .get();
        
        teamSales += memberCommissions.docs.reduce((sum, doc) => {
          const commission = doc.data() as AgentCommission;
          return sum + commission.orderAmount;
        }, 0);
      }
