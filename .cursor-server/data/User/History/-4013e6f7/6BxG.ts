...
class CVVService {
  private apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5001';
   ...
   * 批量處理 CVV 數據
   */
  async processBatch(batchId: string): Promise<CVVBatch> {
    try {
      const response = await fetch(`${this.apiBaseUrl}/api/cvv/batch/${batchId}/process`, {
        method: 'POST',
      });

      const result: CVVApiResponse<CVVBatch> = await response.json();
      
      if (!result.success) {
        throw new Error(result.error || '批次處理失敗');
      }
   ...
   * 驗證 CVV 卡片數據
   */
  async validateCards(cards: Partial<CVVCard>[]): Promise<{
    valid: CVVCard[];
    invalid: { card: Partial<CVVCard>; errors: string[] }[];
  }> {
    try {
      const response = await fetch(`${this.apiBaseUrl}/api/cvv/validate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('CVV 卡片驗證失敗:', error);
      throw error;
    }
  }
